...
namespace IceCream.Serialization
{
    ...
    public class SceneSerializer
    {
        ...
        public static void SerializeScene(SceneBase scene, string filename)
        {
            try
            {
                ...
                if (scene.Equals(SceneManager.GlobalDataHolder))
                {
                    ...
                    XmlNode _node = _doc.CreateElement("AutoLoginToLive");
                    _node.InnerText = globalDataHolder.AutoSignIntoLive.ToString();
        ...
        private static void WriteSpriteItemBase(XmlNode itemNode, XmlDocument doc, Sprite item)
        {
            ...
            itemNode.AppendChildIfNotNull(WriteProperty("AutoScroll", item, doc));
            itemNode.AppendChildIfNotNull(WriteProperty("AutoScrollArea", item, doc));
            itemNode.AppendChildIfNotNull(WriteProperty("AutoScrollMirroring", item, doc));
            itemNode.AppendChildIfNotNull(WriteProperty("AutoScrollSpeed", item, doc));
            itemNode.AppendChildIfNotNull(WriteProperty("AutoScrollVertical", item, doc));
        }
        ...
        private static void WriteSceneItemBase(XmlNode node, XmlDocument doc, SceneItem item)
        {
            ...
            node.AppendChildIfNotNull(WriteProperty("ObjectType", item, doc));
            node.AppendChildIfNotNull(WriteProperty("Tags", item, doc));
            node.AppendChildIfNotNull(WriteProperty("FactionId", item, doc));
        }
        ...
        private static XmlNode WriteProperty(string name, object item, XmlDocument doc, object defaultValue)
        {
            ...
            if (_prop.CanRead && _prop.CanWrite)
            {
                ...
                else if (_prop.PropertyType == typeof(List<string>)) {
                    List<string> value = (List<string>)_prop.GetValue(item, null);

                    foreach (string s in value) {
                        if (!string.IsNullOrEmpty(s)) {
                            XmlNode _tagNode = _node.AppendChild(doc.CreateElement("Tag"));
                            _tagNode.InnerText = s;
                        }
                    }
                }
        ...
        public static void DeSerializeScene(String filename, String contentRoot, SceneBase scene)
        {
            ...
            if (scene is GlobalDataHolder)
            {
                ...
                _resNode = doc.SelectSingleNode("IceScene/AutoLoginToLive");
                bool autoSignIn = true;
                if (_resNode != null) {
                    bool.TryParse(_resNode.InnerText, out autoSignIn);
                }
                gdh.AutoSignIntoLive = autoSignIn;
            }
        ...
        private static void SetProperty(string p, object targetObject, XmlNode node, object defaultValue)
        {
            ...
            if (_newNode == null)
            {
                ...
                else if (_prop.PropertyType == typeof(uint)) {
                    _prop.SetValue(targetObject, 0, null);
                }
                else if (_prop.PropertyType == typeof(String))
                {
            ...
            else if (_prop.PropertyType == typeof(int))
            {
                ...
                _prop.SetValue(targetObject, uint.Parse(_newNode.InnerText, CultureInfo.InvariantCulture), null);
            }
            else if (_prop.PropertyType == typeof(string))
            {
                ...
                SceneItem sceneItem = targetObject as SceneItem;
                foreach (XmlNode tagNode in _newNode.ChildNodes) {
                    sceneItem.Tags.Add(tagNode.InnerText);
                }
            }
            else if (_prop.PropertyType.BaseType == typeof(Enum))
            {
        ...
        private static Sprite LoadSprite(XmlNode _node, SceneBase scene)
        {
            ...
            SetProperty("Tint", _sprite, _node);
            SetProperty("BlendingType", _sprite, _node);
            SetProperty("UseTilingSafeBorders", _sprite, _node);

            SetProperty("AutoScroll", _sprite, _node);
            SetProperty("AutoScrollArea", _sprite, _node);
            SetProperty("AutoScrollMirroring", _sprite, _node);
            SetProperty("AutoScrollSpeed", _sprite, _node);
            SetProperty("AutoScrollVertical", _sprite, _node);
            return _sprite;
        }
        ...
        private static void LoadBaseSceneItem(XmlNode node, object item)
        {
            ...
            foreach (XmlNode _siNode in node.ChildNodes)
            {
                ...
                if (_prop != null)
                {
                    ...
                    {
                        if (_prop.Name == "SourceRectangle" || _prop.Name == "Tags")
                        {
